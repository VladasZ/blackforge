    ...
    public static class Competitive
    {
        ...
        public static void Expect(string serverName, Ticket ticket)
        {
            ...
            waiting = new Waiting
            {
                server = serverName,
                world = ticket.world,
                forbidden = Tiers.Expand(ticket.forbidden, log.LogWarning),
            };
        ...
        private static GameObject Panel(FejdStartup menu, List<ItemDrop.ItemData> found)
        {
            ...
            List<string> names = found
                ...
                .ThenBy(name => name)
                .ToList();
            int shown = Math.Min(names.Count, ShownMax);
            float padding = 16f;
            float titleHeight = 64f;
            float height = padding * 2 + titleHeight + RowHeight * (shown + (names.Count > shown ? 1 : 0));
            ...
            for (int i = 0; i < shown; i++)
            {
                ...
                float y = -padding - titleHeight - RowHeight * i;
                ...
                iconRect.anchoredPosition = new Vector2(padding, y - 2f);
                iconRect.sizeDelta = new Vector2(IconSize, IconSize);
                Text(root, model, $"{Tiers.Title(name)}, needs {waiting.forbidden[name].boss}",
                    RowTextSize, new Vector2(padding + IconSize + 10f, y),
                    new Vector2(PanelWidth - padding * 2 - IconSize - 10f, RowHeight));
            }
            if (names.Count > shown)
            {
                Text(root, model, $"and {names.Count - shown} more", RowTextSize,
                    new Vector2(padding, -padding - titleHeight - RowHeight * shown),
                    new Vector2(PanelWidth - padding * 2, RowHeight));
            }
            return root;
        }
