...
fn main() -> Result<()> {
    ...
    let script = format!(
        r#"set -euo pipefail
cargo build --locked --release -p blackforge --bin blackforge-gui
BIN=target/release-linux/release/blackforge-gui
OUT={stage}/out
rm -rf $OUT /tmp/out && mkdir -p $OUT /tmp/out
cargo deb -p blackforge --no-build --no-strip -o $OUT/{name}.deb
cd /tmp/out
linuxdeploy --appdir AppDir -e /work/apps/app/$BIN -d /work/apps/app/{stage}/{name}.desktop -i /work/apps/app/{stage}/{name}.svg --output appimage
cd /work/apps/app
...
cp $BIN $OUT/{name}"#,
    ...
    copy(format!("{out}/{}.deb", r.name), &deb)?;
    copy(format!("{out}/{}.AppImage", r.name), &appimage)?;
    copy(format!("{out}/{}", r.name), &bare)?;
    run(&format!(
        "cargo run --locked --manifest-path build/Cargo.toml --bin release-sign -- {bare} {appimage}"
    ))?;
...
fn write_desktop_file(r: &Release) -> Result<()> {
    copy(
        "assets/images/nav_game.svg",
        format!("{STAGE}/{}.svg", r.name),
    )?;
    write(
