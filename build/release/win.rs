...
cargo xwin build --locked --release -p blackforge --bin blackforge-gui --target {triple}
makensis -DNAME={name} -DVERSION={version} -DEXE=/work/apps/app/target/release-win/{triple}/release/blackforge-gui.exe -DOUTPUT=/work/apps/app/{stage}/{name}-{arch}-setup.exe build/release/installer.nsi
cp /work/apps/app/target/release-win/{triple}/release/blackforge-gui.exe /work/apps/app/{stage}/{name}-{arch}.exe
"#,
            name = r.name,
            version = r.version
        ));
    ...
    for (arch, _) in &targets {
        ...
        copy(format!("{stage}/{}-{arch}-setup.exe", r.name), &setup)?;
        copy(format!("{stage}/{}-{arch}.exe", r.name), &bare)?;
        run(&format!(
