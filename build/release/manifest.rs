...
fn main() -> Result<()> {
    ...
    if args.iter().any(|arg| arg == "--complete") {
        for suffix in [
            "mac-universal.dmg",
            "macos-universal",
    ...
    ensure!(!updater.platforms.is_empty(), "no signed update artifacts");
    ...
    write("dist/manifest.json", format!("{text}\n"))?;
    println!("{text}");

    let text = serde_json::to_string_pretty(&updater)?;
    write("dist/updater.json", format!("{text}\n"))?;
    println!("{text}");
    Ok(())
}
...
fn updater_manifest(r: &Release, has: &dyn Fn(&str) -> Option<String>) -> Result<UpdaterManifest> {
    ...
    for (key, suffix) in entries {
        ...
        let text = read_to_string(&meta_path).with_context(|| format!("read {meta_path}"))?;
